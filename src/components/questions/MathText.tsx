      ...
      {items.map((item, i) => {
        ...
        return (
          <div key={i} className="flex gap-3 items-start">
            <span className="shrink-0 min-w-[1.75rem] text-right font-bold text-accent mt-0.5">{item.label}</span>
            <span className="flex-1 min-w-0"><MathTextInline text={mainContent} /></span>
            {marks && <span className="shrink-0 ml-auto text-xs font-medium text-foreground/40 whitespace-nowrap self-start mt-0.5">[{marks} marks]</span>}
          </div>
        );
      })}
    </div>
