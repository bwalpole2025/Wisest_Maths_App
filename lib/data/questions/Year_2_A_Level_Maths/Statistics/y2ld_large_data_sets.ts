...
export const questions: Question[] = [
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find the midpoint \\( x \\) of each class.", workingLatex: "x=\\frac{100+120}{2}=110,\\ 130,\\ 150,\\ \\frac{160+180}{2}=170", explanation: "Use class midpoints for every calculation; using boundaries would distort both mean and spread." },
        { stepNumber: 2, description: "Build the \\( fx \\) column.", workingLatex: "8(110)=880,\\ 18(130)=2340,\\ 16(150)=2400,\\ 8(170)=1360", explanation: "Frequency times midpoint for each class." },
        { stepNumber: 3, description: "Sum \\( fx \\) and \\( f \\).", workingLatex: "\\sum fx=880+2340+2400+1360=6980,\\quad \\sum f=50", explanation: "Totals needed for the mean; \\( \\sum f=50 \\) matches the number of apples." },
        { stepNumber: 4, description: "Estimate the mean.", workingLatex: "\\bar{x}=\\frac{6980}{50}=139.6", explanation: "Total \\( fx \\) over total frequency." },
        { stepNumber: 5, description: "Build the \\( fx^2 \\) column.", workingLatex: "8(110^2)=96800,\\ 18(130^2)=304200,\\ 16(150^2)=360000,\\ 8(170^2)=231200", explanation: "Square each midpoint first, then multiply by frequency; a common slip is to square \\( fx \\) instead of \\( x \\)." },
        { stepNumber: 6, description: "Sum \\( fx^2 \\).", workingLatex: "\\sum fx^2=96800+304200+360000+231200=992200", explanation: "This estimates the sum of the squared masses." },
        { stepNumber: 7, description: "Apply the standard deviation formula and round.", workingLatex: "\\sigma=\\sqrt{\\frac{\\sum fx^2}{\\sum f}-\\bar{x}^2}=\\sqrt{\\frac{992200}{50}-139.6^2}=\\sqrt{19844-19488.16}=\\sqrt{355.84}\\approx18.9", explanation: "Mean of the squares minus the square of the mean, then square root; keep the unrounded mean inside the square to avoid rounding error." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Build the cumulative frequencies.", workingLatex: "12,\\ 12{+}20{=}32,\\ 32{+}28{=}60,\\ 60{+}14{=}74,\\ 74{+}6{=}80", explanation: "Running totals used to locate both quartiles." },
        { stepNumber: 2, description: "Find the lower-quartile position and class.", workingLatex: "\\frac{n}{4}=\\frac{80}{4}=20\\ \\Rightarrow\\ 12<20\\le32\\ \\Rightarrow\\ 5\\le t<10", explanation: "The 20th value lies in the second class (cumulative frequency passes 20 here)." },
        { stepNumber: 3, description: "Substitute into the interpolation formula for \\( Q_1 \\).", workingLatex: "Q_1=L+\\frac{\\frac{n}{4}-F}{f}\\times w=5+\\frac{20-12}{20}\\times5", explanation: "Here \\( L=5,\\ F=12,\\ f=20,\\ w=5 \\)." },
        { stepNumber: 4, description: "Evaluate \\( Q_1 \\).", workingLatex: "Q_1=5+\\frac{8}{20}\\times5=5+2=7", explanation: "The 20th value is \\( \\tfrac{8}{20} \\) of the way through the class." },
        { stepNumber: 5, description: "Find the upper-quartile position and class.", workingLatex: "\\frac{3n}{4}=\\frac{240}{4}=60\\ \\Rightarrow\\ 32<60\\le60\\ \\Rightarrow\\ 10\\le t<15", explanation: "The 60th value lies in the third class; \\( L=10,\\ F=32,\\ f=28,\\ w=5 \\)." },
        { stepNumber: 6, description: "Substitute and evaluate \\( Q_3 \\).", workingLatex: "Q_3=10+\\frac{60-32}{28}\\times5=10+\\frac{28}{28}\\times5=15", explanation: "The numerator equals the class frequency, so \\( Q_3 \\) sits at the top boundary of the class." },
        { stepNumber: 7, description: "Compute the interquartile range.", workingLatex: "\\text{IQR}=Q_3-Q_1=15-7=8.0", explanation: "Subtract the lower quartile from the upper quartile." }
      ],
  ...
  <Polygon points={[[8,0],[8,2],[14,2],[14,0]]} color="var(--mafs-fg-blue)" fillOpacity={0.2} />
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "State how to recover a frequency from a density.", workingLatex: "f=\\text{density}\\times\\text{width}", explanation: "Each bar's frequency is its height (density) times its class width." },
        { stepNumber: 2, description: "Find the widths of the three known classes.", workingLatex: "2-0=2,\\ 6-2=4,\\ 8-6=2", explanation: "Upper minus lower boundary for each class." },
        { stepNumber: 3, description: "Compute the three known frequencies.", workingLatex: "9\\times2=18,\\ 12\\times4=48,\\ 15\\times2=30", explanation: "Density times width for each known bar." },
        { stepNumber: 4, description: "Find the missing frequency from the total.", workingLatex: "120-(18+48+30)=120-96=24", explanation: "The frequencies must sum to the stated total of 120." },
        { stepNumber: 5, description: "Find the width of the final class.", workingLatex: "14-8=6", explanation: "Class width for \\( 8\\le m<14 \\)." },
        { stepNumber: 6, description: "Compute its frequency density.", workingLatex: "\\text{density}=\\frac{f}{\\text{width}}=\\frac{24}{6}=4", explanation: "Frequency divided by class width gives the missing bar height." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find the midpoint of each class.", workingLatex: "x=\\frac{0+5}{2}=2.5,\\ \\frac{5+10}{2}=7.5,\\ \\frac{10+20}{2}=15,\\ \\frac{20+40}{2}=30", explanation: "The midpoint is still the average of the boundaries even when widths differ; do not assume equal spacing." },
        { stepNumber: 2, description: "Build the \\( fx \\) column and sum.", workingLatex: "8(2.5)=20,\\ 12(7.5)=90,\\ 14(15)=210,\\ 6(30)=180;\\ \\sum fx=500", explanation: "Frequency times midpoint, then total." },
        { stepNumber: 3, description: "Estimate the mean.", workingLatex: "\\bar{x}=\\frac{\\sum fx}{\\sum f}=\\frac{500}{40}=12.5", explanation: "Total \\( fx \\) over total frequency 40." },
        { stepNumber: 4, description: "Build the \\( fx^2 \\) column and sum.", workingLatex: "8(6.25)=50,\\ 12(56.25)=675,\\ 14(225)=3150,\\ 6(900)=5400;\\ \\sum fx^2=9275", explanation: "Square each midpoint, multiply by frequency, then total." },
        { stepNumber: 5, description: "Apply the standard deviation formula and round.", workingLatex: "\\sigma=\\sqrt{\\frac{\\sum fx^2}{\\sum f}-\\bar{x}^2}=\\sqrt{\\frac{9275}{40}-12.5^2}=\\sqrt{231.875-156.25}=\\sqrt{75.625}\\approx8.7", explanation: "Mean of squares minus mean squared, then square root." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find the position of the 90th percentile.", workingLatex: "P_{90}\\ \\text{position}=\\frac{90}{100}\\times n=0.9\\times200=180", explanation: "The 90th percentile is at \\( \\frac{90}{100}n \\); for grouped data we use \\( 0.9n \\), not \\( 0.9(n+1) \\)." },
        { stepNumber: 2, description: "Build the cumulative frequencies.", workingLatex: "24,\\ 74,\\ 140,\\ 182,\\ 200", explanation: "Running totals to locate the 180th value." },
        { stepNumber: 3, description: "Identify the class containing the 180th value.", workingLatex: "140<180\\le182\\ \\Rightarrow\\ 60\\le x<80", explanation: "Cumulative frequency rises from 140 to 182 across this class, so the 180th value lies here." },
        { stepNumber: 4, description: "Read off the interpolation quantities.", workingLatex: "L=60,\\ F=140,\\ f=42,\\ w=20", explanation: "Lower boundary, cumulative frequency before the class, class frequency and width." },
        { stepNumber: 5, description: "Write the formula and substitute.", workingLatex: "P_{90}=L+\\frac{0.9n-F}{f}\\times w=60+\\frac{180-140}{42}\\times20", explanation: "Same interpolation structure as the median but with position \\( 0.9n \\)." },
        { stepNumber: 6, description: "Evaluate and round to 1 d.p.", workingLatex: "60+\\frac{40}{42}\\times20=60+19.05=79.0", explanation: "The percentile sits about 19.05 marks into the \\( 60\\le x<80 \\) class." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find the midpoints and build the \\( fx \\) column.", workingLatex: "x=12.5,13.5,14.5,15.5,16.5;\\ fx=50,\\ 175.5,\\ 261,\\ 155,\\ 82.5", explanation: "Each midpoint is the average of its boundaries; multiply by frequency for each class." },
        { stepNumber: 2, description: "Sum \\( fx \\) and estimate the mean.", workingLatex: "\\sum fx=50+175.5+261+155+82.5=724;\\ \\bar{x}=\\frac{724}{50}=14.48", explanation: "Total \\( fx \\) over total frequency 50 gives the mean to 2 d.p." },
        { stepNumber: 3, description: "Build cumulative frequencies and locate the median class.", workingLatex: "4,\\ 17,\\ 35,\\ 45,\\ 50;\\ \\frac{n}{2}=25\\ \\Rightarrow\\ 17<25\\le35\\ \\Rightarrow\\ 14\\le t<15", explanation: "The 25th value lies in the third class, where the cumulative frequency passes 25." },
        { stepNumber: 4, description: "Apply linear interpolation for the median.", workingLatex: "\\text{median}=L+\\frac{\\frac{n}{2}-F}{f_m}\\times w=14+\\frac{25-17}{18}\\times1=14+0.44=14.44", explanation: "Here \\( L=14,\\ F=17,\\ f_m=18,\\ w=1 \\); the narrow class width of 1 makes the fractional part small." },
        { stepNumber: 5, description: "Compare the mean and median to comment on skew.", workingLatex: "\\bar{x}=14.48>\\text{median}=14.44", explanation: "The mean exceeds the median, indicating a slight positive (right) skew, since the longer tail pulls the mean upward." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Write down the quartiles.", workingLatex: "Q_1 = 14, \\qquad Q_3 = 22", explanation: "List the given values first so the substitutions are easy to track. Both are in \\( ^\\circ\\text{C} \\)." },
        { stepNumber: 2, description: "Find the interquartile range.", workingLatex: "\\text{IQR} = Q_3 - Q_1 = 22 - 14 = 8", explanation: "The IQR is the difference between the upper and lower quartiles. A common slip is to subtract the wrong way round and get a negative IQR." },
        { stepNumber: 3, description: "Write the upper-fence formula.", workingLatex: "Q_3 + 1.5\\times\\text{IQR}", explanation: "The high-outlier boundary is \\( 1.5 \\) IQRs above \\( Q_3 \\)." },
        { stepNumber: 4, description: "Substitute and evaluate the upper fence.", workingLatex: "22 + 1.5\\times 8 = 22 + 12 = 34", explanation: "Compute \\( 1.5\\times 8 = 12 \\) first, then add. Any temperature above \\( 34\\,^\\circ\\text{C} \\) is a high outlier." },
        { stepNumber: 5, description: "Write the lower-fence formula.", workingLatex: "Q_1 - 1.5\\times\\text{IQR}", explanation: "The low-outlier boundary is \\( 1.5 \\) IQRs below \\( Q_1 \\)." },
        { stepNumber: 6, description: "Substitute and evaluate the lower fence.", workingLatex: "14 - 1.5\\times 8 = 14 - 12 = 2", explanation: "Any temperature below \\( 2\\,^\\circ\\text{C} \\) is a low outlier. Don't forget to compute this fence too, even when only the upper one matters here." },
        { stepNumber: 7, description: "Compare \\( 35\\,^\\circ\\text{C} \\) with the fences.", workingLatex: "2 \\le 35 \\;\\text{?} \\qquad 35 > 34", explanation: "\\( 35 \\) is well above the lower fence, but it exceeds the upper fence of \\( 34\\,^\\circ\\text{C} \\)." },
        { stepNumber: 8, description: "State the conclusion in context.", workingLatex: "35 > 34 \\Rightarrow \\text{outlier}", explanation: "Since \\( 35\\,^\\circ\\text{C} \\) lies above the upper fence, it is classified as an outlier. Always interpret the result in context rather than just stating 'yes'." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Write down the mean and standard deviation.", workingLatex: "\\bar{x} = 4.6, \\qquad \\sigma = 0.5", explanation: "List the summary statistics so the limits can be built cleanly. Both are in kg." },
        { stepNumber: 2, description: "Write the upper-limit formula.", workingLatex: "\\bar{x} + 2\\sigma", explanation: "The acceptable region is within two standard deviations of the mean, so the top boundary is \\( \\bar{x}+2\\sigma \\)." },
        { stepNumber: 3, description: "Substitute and evaluate the upper limit.", workingLatex: "4.6 + 2\\times 0.5 = 4.6 + 1.0 = 5.6", explanation: "Masses above \\( 5.6 \\) kg lie more than two standard deviations above the mean. Slip to avoid: forgetting the factor of 2 and using just one \\( \\sigma \\)." },
        { stepNumber: 4, description: "Write the lower-limit formula.", workingLatex: "\\bar{x} - 2\\sigma", explanation: "The bottom boundary is \\( \\bar{x}-2\\sigma \\)." },
        { stepNumber: 5, description: "Substitute and evaluate the lower limit.", workingLatex: "4.6 - 2\\times 0.5 = 4.6 - 1.0 = 3.6", explanation: "Masses below \\( 3.6 \\) kg lie more than two standard deviations below the mean." },
        { stepNumber: 6, description: "Compare \\( 5.8 \\) kg with the limits.", workingLatex: "3.6 \\le 5.8 \\;\\text{?} \\qquad 5.8 > 5.6", explanation: "\\( 5.8 \\) is above the lower limit but exceeds the upper limit of \\( 5.6 \\) kg." },
        { stepNumber: 7, description: "State the conclusion in context.", workingLatex: "5.8 > 5.6 \\Rightarrow \\text{outlier}", explanation: "Since the lamb's mass lies more than two standard deviations above the mean mass, it is an outlier. Answer in context, naming the variable (mass of the lamb)." }
      ],
  ...
  <Text x={20} y={0.24} attach="s">20</Text>
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 2, description: "Compare the medians (location).", workingLatex: "\\text{A: } Q_2 = 9 \\quad \\text{vs} \\quad \\text{B: } Q_2 = 14", explanation: "Site B has a higher median wind speed than Site A, so on a typical day Site B is windier. A comparison must always name which site is greater and in context." },
        { stepNumber: 3, description: "Find each IQR (spread).", workingLatex: "\\text{IQR}_A = 13 - 6 = 7, \\qquad \\text{IQR}_B = 17 - 9 = 8", explanation: "Compute the central spread for each site separately before comparing." },
        { stepNumber: 4, description: "Compare the IQRs in context.", workingLatex: "\\text{IQR}_B = 8 > 7 = \\text{IQR}_A", explanation: "Site B has a slightly larger IQR, so the central wind speeds are a little more spread out at Site B." },
        { stepNumber: 5, description: "Compare the ranges.", workingLatex: "\\text{Range}_A = 20 - 2 = 18, \\qquad \\text{Range}_B = 28 - 3 = 25", explanation: "Site B has the greater overall range, confirming more variability in wind speed. Mentioning both IQR and range gives a fuller spread comparison." },
        { stepNumber: 6, description: "Assess skewness of Site A.", workingLatex: "Q_2 - Q_1 = 9 - 6 = 3, \\qquad Q_3 - Q_2 = 13 - 9 = 4", explanation: "Since \\( Q_3 - Q_2 > Q_2 - Q_1 \\), the upper half of the box is longer, so Site A's wind speeds are positively skewed." },
        { stepNumber: 7, description: "Assess skewness of Site B.", workingLatex: "Q_2 - Q_1 = 14 - 9 = 5, \\qquad Q_3 - Q_2 = 17 - 14 = 3", explanation: "Since \\( Q_2 - Q_1 > Q_3 - Q_2 \\), the lower half of the box is longer, so Site B's wind speeds are negatively skewed. Slip: reading the skew direction backwards." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Locate Q1 and interpolate.", workingLatex: "\\tfrac{1}{4}\\times 120 = 30 \\Rightarrow Q_1 = 2 + \\frac{30-18}{48-18}\\times 2 = 2 + \\tfrac{12}{30}\\times 2 = 2.8", explanation: "CF = 30 lies in (2, 4], where CF goes 18 → 48 over width 2. \\( Q_1 \\approx 2.8 \\) mm." },
        { stepNumber: 2, description: "Locate the median and interpolate.", workingLatex: "\\tfrac{1}{2}\\times 120 = 60 \\Rightarrow Q_2 = 4 + \\frac{60-48}{84-48}\\times 2 = 4 + \\tfrac{12}{36}\\times 2 \\approx 4.67", explanation: "CF = 60 lies in (4, 6], where CF goes 48 → 84. The median is about 4.67 mm." },
        { stepNumber: 3, description: "Locate Q3 and interpolate.", workingLatex: "\\tfrac{3}{4}\\times 120 = 90 \\Rightarrow Q_3 = 8 + \\frac{90-84}{108-84}\\times 2 = 8 + \\tfrac{6}{24}\\times 2 = 8.5", explanation: "CF = 90 lies in (8, 10]... but note CF(8) = 108 already exceeds 90, so Q3 actually lies in (6, 8]." },
        { stepNumber: 4, description: "Correct the Q3 interval and re-interpolate.", workingLatex: "Q_3 = 6 + \\frac{90-84}{108-84}\\times 2 = 6 + \\tfrac{6}{24}\\times 2 = 6.5", explanation: "CF rises from 84 (at 6 mm) to 108 (at 8 mm), so CF = 90 falls in (6, 8]. The original used the wrong lower endpoint (8 instead of 6); the correct \\( Q_3 = 6.5 \\) mm." },
        { stepNumber: 5, description: "Compute the IQR and the upper outlier fence.", workingLatex: "\\text{IQR} = 6.5 - 2.8 = 3.7, \\quad Q_3 + 1.5\\times\\text{IQR} = 6.5 + 1.5\\times 3.7 = 6.5 + 5.55 = 12.05", explanation: "IQR = 3.7 mm; the upper fence is \\( 6.5 + 5.55 = 12.05 \\) mm, so daily totals above about 12.05 mm would be outliers." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Back-transform the mean of x.", workingLatex: "\\bar{x} = 2\\bar{y} + 5 = 2\\times 4 + 5 = 13", explanation: "Invert the coding \\( y=\\tfrac{x-5}{2} \\) by multiplying by 2 and adding 5. The mean rainfall is 13 mm." },
        { stepNumber: 2, description: "Back-transform the standard deviation of x.", workingLatex: "\\sigma_x = 2\\sigma_y = 2\\times 1.5 = 3", explanation: "Only the scale factor of 2 affects the spread; the '+5' does not. So \\( \\sigma_x = 3 \\) mm." },
        { stepNumber: 3, description: "Write the lower acceptable limit.", workingLatex: "\\bar{x} - 2\\sigma_x = 13 - 2\\times 3 = 13 - 6 = 7", explanation: "Totals below 7 mm would lie more than two standard deviations below the mean and be outliers." },
        { stepNumber: 4, description: "Write the upper acceptable limit.", workingLatex: "\\bar{x} + 2\\sigma_x = 13 + 2\\times 3 = 13 + 6 = 19", explanation: "Totals above 19 mm would be outliers. Slip: using the coded \\( \\bar{y}, \\sigma_y \\) here instead of the back-transformed values." },
        { stepNumber: 5, description: "State the non-outlier range in context.", workingLatex: "7 \\le x \\le 19", explanation: "Rainfall totals between 7 mm and 19 mm are not classified as outliers." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Compare the medians.", workingLatex: "\\text{X: } 11 \\quad \\text{vs} \\quad \\text{Y: } 12", explanation: "Airfield Y has a slightly higher median wind speed, so is typically marginally windier." },
        { stepNumber: 2, description: "Find each IQR.", workingLatex: "\\text{IQR}_X = 16 - 7 = 9, \\qquad \\text{IQR}_Y = 15 - 9 = 6", explanation: "Compute the central spread for each airfield before comparing." },
        { stepNumber: 3, description: "Compare the IQRs in context.", workingLatex: "\\text{IQR}_X = 9 > 6 = \\text{IQR}_Y", explanation: "Airfield X has the larger IQR, so although Y is typically a touch windier, X shows greater day-to-day variability in wind speed." },
        { stepNumber: 4, description: "Use the given count for Airfield X.", workingLatex: "\\text{days above } 14 = 30", explanation: "30 of the 100 days at Airfield X had wind speed above 14 knots." },
        { stepNumber: 5, description: "Find days at most 14 knots.", workingLatex: "100 - 30 = 70", explanation: "Subtract from the 100-day total. Therefore 70 days had wind speed at most 14 knots at Airfield X. Slip: subtracting from the wrong total or confusing 'above' with 'at most'." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Check the strata sizes total the population.", workingLatex: "200+260+180+200 = 840 = N", explanation: "Before allocating, confirm the four faculty counts add to the stated population of \\\\( 840 \\\\); if they did not, one of the figures would be wrong. A common slip is to start computing fractions without this sanity check." },
        { stepNumber: 2, description: "State the stratified sampling rule.", workingLatex: "n_h = \\\\frac{N_h}{N}\\\\times n,\\\\quad N=840,\\\\ n=60", explanation: "Each stratum contributes in proportion to its share of the population. Here \\\\( N_h \\\\) is a faculty size, \\\\( N=840 \\\\) the whole college and \\\\( n=60 \\\\) the sample. The common sampling fraction is \\\\( \\\\tfrac{n}{N}=\\\\tfrac{60}{840}=\\\\tfrac{1}{14} \\\\), so every count is just divided by \\\\( 14 \\\\)." },
        { stepNumber: 3, description: "Arts stratum: form the fraction times n.", workingLatex: "\\\\frac{200}{840}\\\\times 60 = \\\\frac{200}{14} = 14.2857\\\\dots", explanation: "Multiply the Arts size by the sampling fraction. Keep the exact decimal so the rounding direction is unambiguous; truncating early can flip a borderline case." },
        { stepNumber: 4, description: "Round the Arts allocation.", workingLatex: "14.2857\\\\dots \\\\to 14", explanation: "The fractional part \\\\( 0.29 < 0.5 \\\\), so round down to \\\\( 14 \\\\) students. You cannot sample part of a person, so every stratum must be a whole number." },
        { stepNumber: 5, description: "Science stratum: form the fraction times n.", workingLatex: "\\\\frac{260}{840}\\\\times 60 = \\\\frac{260}{14} = 18.5714\\\\dots", explanation: "Same method with the Science size \\\\( 260 \\\\). Science is the largest faculty so it should receive the largest allocation — a useful cross-check on the arithmetic." },
        { stepNumber: 6, description: "Round the Science allocation.", workingLatex: "18.5714\\\\dots \\\\to 19", explanation: "The fractional part \\\\( 0.57 > 0.5 \\\\), so round up to \\\\( 19 \\\\). Forgetting that this rounds up (and writing 18) is the usual error here." },
        { stepNumber: 7, description: "Engineering stratum: form the fraction times n.", workingLatex: "\\\\frac{180}{840}\\\\times 60 = \\\\frac{180}{14} = 12.8571\\\\dots", explanation: "Engineering is the smallest faculty, so expect the smallest allocation." },
        { stepNumber: 8, description: "Round the Engineering allocation.", workingLatex: "12.8571\\\\dots \\\\to 13", explanation: "The fractional part \\\\( 0.86 > 0.5 \\\\), so round up to \\\\( 13 \\\\)." },
        { stepNumber: 9, description: "Business stratum: form the fraction times n and round.", workingLatex: "\\\\frac{200}{840}\\\\times 60 = 14.2857\\\\dots \\\\to 14", explanation: "Business has the same size as Arts \\\\( (200) \\\\), so its allocation is identical, \\\\( 14 \\\\). Equal strata must always receive equal allocations — a quick consistency check." },
        { stepNumber: 10, description: "Check the rounded allocations total n.", workingLatex: "14+19+13+14 = 60\\\\ \\\\checkmark", explanation: "The rounded allocations sum to the required sample size \\\\( 60 \\\\), so no adjustment is needed. Always finish a stratified problem with this total check: if rounding had given 59 or 61 you would adjust the stratum whose decimal was closest to \\\\( .5 \\\\) to restore the total." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 6, description: "Divide the grand total by the combined count.", workingLatex: "\\\\bar{x}=\\\\frac{3433}{50}=68.66", explanation: "The combined mean \\\\( 68.66 \\\\) lies between \\\\( 64.5 \\\\) and \\\\( 71.0 \\\\), and is closer to \\\\( 71.0 \\\\) because station B contributed more readings — a sense-check that the weighting was applied correctly. The result is already exact to 2 d.p." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 3, description: "Collect the numerator and denominator.", workingLatex: "\\\\bar{x}=\\\\frac{0+11+2f+27+16}{30+f}=\\\\frac{54+2f}{30+f}", explanation: "The known contributions \\\\( 11+27+16=54 \\\\) give the constant; the known frequencies \\\\( 6+11+9+4=30 \\\\) give the constant in the denominator." },
        { stepNumber: 4, description: "Set the expression equal to the given mean.", workingLatex: "\\\\frac{54+2f}{30+f}=1.9", explanation: "The stated mean \\\\( 1.9 \\\\) lets us form an equation in the single unknown \\\\( f \\\\)." },
        { stepNumber: 5, description: "Multiply both sides by the denominator.", workingLatex: "54+2f = 1.9(30+f)", explanation: "Clearing the fraction removes \\\\( f \\\\) from the denominator. Forgetting to multiply the \\\\( f \\\\) term inside the bracket is the usual error." },
        { stepNumber: 6, description: "Expand the right-hand side.", workingLatex: "54+2f = 57+1.9f", explanation: "Distribute: \\\\( 1.9\\\\times 30 = 57 \\\\) and \\\\( 1.9\\\\times f = 1.9f \\\\)." },
        { stepNumber: 7, description: "Collect the f terms and solve.", workingLatex: "2f-1.9f = 57-54 \\\\implies 0.1f = 3 \\\\implies f = 30", explanation: "Subtract \\\\( 1.9f \\\\) and \\\\( 54 \\\\) from both sides, then divide by \\\\( 0.1 \\\\). Since \\\\( f \\\\) is a frequency, it must be a non-negative whole number, and \\\\( 30 \\\\) qualifies." },
        { stepNumber: 8, description: "Substitute back to verify.", workingLatex: "\\\\frac{54+2(30)}{30+30}=\\\\frac{114}{60}=1.9\\\\ \\\\checkmark", explanation: "Putting \\\\( f=30 \\\\) back into the original fraction reproduces the given mean exactly, confirming the solution." }
      ],
      finalAnswer: "\\( f = 30 \\).",
      canonicalAnswer: "30",
    },
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "(a) Compute the sampling interval.", workingLatex: "k=\\\\frac{N}{n}=\\\\frac{4500}{150}=30", explanation: "The interval is population size divided by sample size. Here it divides exactly, so every \\\\( 30\\\\text{th} \\\\) parcel is selected. (If \\\\( k \\\\) were not a whole number you would round it.)" },
        { stepNumber: 2, description: "(b) Describe the random start.", workingLatex: "\\\\text{start } r \\\\in \\\\{1,2,\\\\dots,30\\\\}\\\\ \\\\text{chosen at random}", explanation: "The first member is picked at random from the first interval \\\\( 1 \\\\) to \\\\( k=30 \\\\) (e.g. using a random-number generator). Choosing within the first interval guarantees the whole list is covered and removes starting-point bias." },
        { stepNumber: 3, description: "Apply the given start r = 17 by adding k repeatedly.", workingLatex: "r,\\\\ r+k,\\\\ r+2k = 17,\\\\ 17+30,\\\\ 17+60", explanation: "Successive selections are spaced exactly \\\\( k=30 \\\\) apart starting from \\\\( r=17 \\\\)." },
        { stepNumber: 4, description: "Evaluate the first three positions.", workingLatex: "17,\\\\ 47,\\\\ 77", explanation: "These are the database positions of the first three audited parcels. A common slip is to start counting at \\\\( 30 \\\\) rather than at the random start \\\\( r \\\\)." },
        { stepNumber: 5, description: "Confirm all 150 selections fit inside the list.", workingLatex: "r+(n-1)k = 17+30(149)=17+4470=4487\\\\le 4500", explanation: "The last (150th) selection is position \\\\( 4487 \\\\), which is within the \\\\( 4500 \\\\) parcels, so the systematic scheme yields exactly \\\\( 150 \\\\) valid selections." },
        { stepNumber: 6, description: "(c) State an advantage.", workingLatex: "\\\\text{quick to run; even spread through the ordered list}", explanation: "Systematic sampling is quick to administer and spreads the sample evenly across the whole database, without needing \\\\( 150 \\\\) separate random numbers as simple random sampling would." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Correct the sum of values.", workingLatex: "\\\\sum x_{\\\\text{new}}=648-38+83=693", explanation: "Remove the wrong value \\\\( 38 \\\\) and add the correct one \\\\( 83 \\\\). The count stays at \\\\( 12 \\\\) because we replace one reading, not add a new one." },
        { stepNumber: 2, description: "Compute the corrected mean.", workingLatex: "\\\\bar{x}=\\\\frac{693}{12}=57.75", explanation: "Divide the corrected total by \\\\( n=12 \\\\). The correction raised the sum by \\\\( 45 \\\\), so the mean rose by \\\\( \\\\tfrac{45}{12}=3.75 \\\\) from the original \\\\( 54 \\\\)." },
        { stepNumber: 3, description: "Correct the sum of squares.", workingLatex: "\\\\sum x^2_{\\\\text{new}}=38000-38^2+83^2", explanation: "The squared-terms total must be adjusted with the squares of the old and new values, not the values themselves. A frequent error is to subtract \\\\( 38 \\\\) and add \\\\( 83 \\\\) here instead of their squares." },
        { stepNumber: 4, description: "Evaluate the squares and the corrected sum of squares.", workingLatex: "38^2=1444,\\\\ 83^2=6889 \\\\implies 38000-1444+6889=43445", explanation: "Carefully square each value, then combine." },
        { stepNumber: 5, description: "State the variance formula.", workingLatex: "\\\\sigma^2=\\\\frac{\\\\sum x^2}{n}-\\\\bar{x}^2", explanation: "Population variance from summary statistics: mean of the squares minus the square of the mean." },
        { stepNumber: 6, description: "Substitute the corrected totals.", workingLatex: "\\\\sigma^2=\\\\frac{43445}{12}-57.75^2=3620.417-3335.0625", explanation: "Evaluate each term separately: \\\\( \\\\tfrac{43445}{12}=3620.417 \\\\) and \\\\( 57.75^2=3335.0625 \\\\). Keep full precision until the final rounding." },
        { stepNumber: 7, description: "Compute the variance and take the square root.", workingLatex: "\\\\sigma^2=285.354,\\\\quad \\\\sigma=\\\\sqrt{285.354}=16.892\\\\dots", explanation: "Subtract to get the variance, then take the positive square root for the standard deviation." },
        { stepNumber: 8, description: "Round to 3 significant figures.", workingLatex: "\\\\sigma \\\\approx 16.9", explanation: "Three significant figures as requested. The standard deviation is large because the corrected value \\\\( 83 \\\\) is far above the mean." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 5, description: "Identify the median.", workingLatex: "\\\\text{position } \\\\tfrac{n+1}{2}=\\\\tfrac{8}{2}=4 \\\\implies \\\\text{4th value} = 23", explanation: "With \\\\( n=7 \\\\) (odd) the median is the single middle value, the 4th, which is \\\\( 23 \\\\). It coincidentally equals the mean here, but in general the two need not match." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 3, description: "Evaluate the original sum of squares.", workingLatex: "\\\\sum x^2 = 20(4.2^2+15.4^2)=20(17.64+237.16)=20(254.8)=5096", explanation: "Square the standard deviation and the mean, add, then multiply by \\\\( n=20 \\\\). A common slip is to use \\\\( \\\\sigma \\\\) instead of \\\\( \\\\sigma^2 \\\\) here." },
        { stepNumber: 4, description: "Remove the outlier from both totals.", workingLatex: "\\\\sum x_{\\\\text{new}}=308-28=280,\\\\quad \\\\sum x^2_{\\\\text{new}}=5096-28^2=5096-784=4312", explanation: "Subtract the value \\\\( 28 \\\\) from the sum and its square \\\\( 784 \\\\) from the sum of squares. The count drops from \\\\( 20 \\\\) to \\\\( 19 \\\\)." },
        { stepNumber: 5, description: "Compute the new mean.", workingLatex: "\\\\bar{x}_{\\\\text{new}}=\\\\frac{280}{19}=14.7368\\\\dots", explanation: "Divide the new total by \\\\( 19 \\\\). The mean falls because the removed value \\\\( 28 \\\\) was above the old mean." },
        { stepNumber: 6, description: "Compute the new variance.", workingLatex: "\\\\sigma^2=\\\\frac{4312}{19}-14.7368^2=226.947-217.174=9.773", explanation: "Apply the variance formula with \\\\( n=19 \\\\), using the unrounded new mean to avoid accumulating error." },
        { stepNumber: 7, description: "Take the square root and round.", workingLatex: "\\\\sigma=\\\\sqrt{9.773}=3.1262\\\\dots \\\\implies \\\\bar{x}\\\\approx 14.7,\\\\ \\\\sigma\\\\approx 3.13", explanation: "The positive square root gives the new standard deviation. Removing the high outlier lowered both the mean and the spread, as expected. Both answers are to 3 significant figures." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Check the strata sizes total the workforce.", workingLatex: "320+280+240+160 = 1000 = N", explanation: "Confirm the four service-length groups add to \\\\( 1000 \\\\) before allocating, so the proportions are valid." },
        { stepNumber: 2, description: "State the allocation rule.", workingLatex: "n_h=\\\\frac{N_h}{N}\\\\times n,\\\\quad N=1000,\\\\ n=80", explanation: "Proportional allocation across the four strata. The common sampling fraction is \\\\( \\\\tfrac{80}{1000}=0.08 \\\\), so each group size is simply multiplied by \\\\( 0.08 \\\\)." },
        { stepNumber: 3, description: "0–5 years: fraction times n, then round.", workingLatex: "\\\\frac{320}{1000}\\\\times 80 = 25.6 \\\\to 26", explanation: "Fractional part \\\\( 0.6 > 0.5 \\\\), so round up to \\\\( 26 \\\\). This is the largest group, so it receives the most places." },
        { stepNumber: 4, description: "5–10 years: fraction times n, then round.", workingLatex: "\\\\frac{280}{1000}\\\\times 80 = 22.4 \\\\to 22", explanation: "Fractional part \\\\( 0.4 < 0.5 \\\\), so round down to \\\\( 22 \\\\)." },
        { stepNumber: 5, description: "10–15 years: fraction times n, then round.", workingLatex: "\\\\frac{240}{1000}\\\\times 80 = 19.2 \\\\to 19", explanation: "Fractional part \\\\( 0.2 < 0.5 \\\\), so round down to \\\\( 19 \\\\)." },
        { stepNumber: 6, description: "15+ years: fraction times n, then round.", workingLatex: "\\\\frac{160}{1000}\\\\times 80 = 12.8 \\\\to 13", explanation: "Fractional part \\\\( 0.8 > 0.5 \\\\), so round up to \\\\( 13 \\\\). This is the smallest group, so it receives the fewest places." },
        { stepNumber: 7, description: "Verify the rounded allocations total n.", workingLatex: "26+22+19+13 = 80\\\\ \\\\checkmark", explanation: "The rounded allocations sum to the required \\\\( 80 \\\\), so the sample is complete with no adjustment needed. Had the rounding given \\\\( 79 \\\\) or \\\\( 81 \\\\), you would tweak the stratum whose decimal was nearest \\\\( .5 \\\\) to fix the total." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 2, description: "Evaluate the known product and clear the denominator.", workingLatex: "2320+67n_2 = 62(40+n_2)", explanation: "Multiply both sides by \\\\( (40+n_2) \\\\); note \\\\( 40\\\\times 58 = 2320 \\\\). Forgetting to distribute the \\\\( 62 \\\\) across both terms is the usual slip." },
        { stepNumber: 3, description: "Expand the right-hand side.", workingLatex: "2320+67n_2 = 2480+62n_2", explanation: "Distribute: \\\\( 62\\\\times 40 = 2480 \\\\) and \\\\( 62\\\\times n_2 = 62n_2 \\\\)." },
        { stepNumber: 4, description: "Collect the n₂ terms.", workingLatex: "67n_2-62n_2 = 2480-2320 \\\\implies 5n_2 = 160", explanation: "Subtract \\\\( 62n_2 \\\\) and \\\\( 2320 \\\\) from both sides to gather the unknown on one side." },
        { stepNumber: 5, description: "Solve for n₂.", workingLatex: "n_2 = \\\\frac{160}{5}=32", explanation: "Divide by \\\\( 5 \\\\). Group 2 has \\\\( 32 \\\\) participants — a sensible whole number, and the combined mean \\\\( 62 \\\\) lies between \\\\( 58 \\\\) and \\\\( 67 \\\\) as it must." },
        { stepNumber: 6, description: "Substitute back to verify.", workingLatex: "\\\\frac{2320+32(67)}{40+32}=\\\\frac{2320+2144}{72}=\\\\frac{4464}{72}=62\\\\ \\\\checkmark", explanation: "Putting \\\\( n_2=32 \\\\) back into the combined-mean formula reproduces \\\\( 62 \\\\) bpm exactly, confirming the answer." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 5, description: "Subtract L and simplify the numerator.", workingLatex: "4 = \\\\frac{10\\\\left(\\\\frac{38+f}{2}-22\\\\right)}{f},\\\\quad \\\\frac{38+f}{2}-22 = \\\\frac{38+f-44}{2}=\\\\frac{f-6}{2}", explanation: "Move \\\\( 20 \\\\) to the left to get \\\\( 24-20=4 \\\\), then combine the fraction inside the bracket over a common denominator of \\\\( 2 \\\\)." },
        { stepNumber: 6, description: "Reduce to a single fraction in f.", workingLatex: "4 = \\\\frac{10\\\\cdot\\\\frac{f-6}{2}}{f}=\\\\frac{5(f-6)}{f}", explanation: "The factor \\\\( \\\\tfrac{10}{2}=5 \\\\) simplifies the expression to \\\\( \\\\tfrac{5(f-6)}{f} \\\\)." },
        { stepNumber: 7, description: "Cross-multiply and solve for f.", workingLatex: "4f = 5(f-6)=5f-30 \\\\implies -f=-30 \\\\implies f = 30", explanation: "Multiply both sides by \\\\( f \\\\), expand, then collect terms. As a frequency, \\\\( f \\\\) must be a non-negative whole number, and \\\\( 30 \\\\) qualifies." },
        { stepNumber: 8, description: "Substitute back to verify.", workingLatex: "N=68,\\\\ \\\\tfrac{N}{2}=34:\\\\ 20+\\\\frac{34-22}{30}\\\\times 10 = 20+\\\\frac{12}{30}\\\\times 10 = 20+4 = 24\\\\ \\\\checkmark", explanation: "With \\\\( f=30 \\\\) the total is \\\\( 68 \\\\), so \\\\( N/2=34 \\\\); the interpolation then gives exactly \\\\( 24 \\\\) minutes, confirming the solution." }
      ],
      finalAnswer: "\\( f = 30 \\).",
      canonicalAnswer: "30",
    },
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find the mid-point of each class.", workingLatex: "x=\\tfrac{0+5}{2}=2.5,\\ \\tfrac{5+10}{2}=7.5,\\ \\tfrac{10+20}{2}=15,\\ \\tfrac{20+30}{2}=25,\\ \\tfrac{30+50}{2}=40", explanation: "The mid-point is the average of the lower and upper class boundaries. Watch the unequal final classes: the width-10 class gives \\(15\\) and the width-20 class gives \\(40\\); a common slip is to reuse a width of 5 throughout." },
        { stepNumber: 2, description: "Multiply each mid-point by its frequency.", workingLatex: "fx = 12(2.5),\\ 18(7.5),\\ 20(15),\\ 8(25),\\ 2(40) = 30,\\ 135,\\ 300,\\ 200,\\ 80", explanation: "These products are the contribution of each class to the total. Keep them listed so the sum can be checked term by term." },
        { stepNumber: 3, description: "Add the products to obtain \\(\\sum fx\\).", workingLatex: "\\sum fx = 30+135+300+200+80 = 745", explanation: "Summing the column gives the estimated grand total of all 60 rainfall readings." },
        { stepNumber: 4, description: "Estimate the mean.", workingLatex: "\\bar{x}=\\dfrac{\\sum fx}{\\sum f}=\\dfrac{745}{60}=12.41\\overline{6}=12.4\\text{ mm (3 s.f.)}", explanation: "Divide \\(\\sum fx\\) by \\(n=60\\). A common slip is dividing by the number of classes (5) instead of the total frequency." },
        { stepNumber: 5, description: "Multiply each squared mid-point by its frequency.", workingLatex: "fx^2 = 12(2.5^2),\\ 18(7.5^2),\\ 20(15^2),\\ 8(25^2),\\ 2(40^2) = 75,\\ 1012.5,\\ 4500,\\ 5000,\\ 3200", explanation: "Square the mid-point first, then multiply by frequency. A frequent error is to square the product \\(fx\\) instead of the mid-point \\(x\\)." },
        { stepNumber: 6, description: "Add to obtain \\(\\sum fx^{2}\\).", workingLatex: "\\sum fx^{2}=75+1012.5+4500+5000+3200=13787.5", explanation: "This running total feeds the variance formula. Carry the exact decimal rather than rounding early." },
        { stepNumber: 7, description: "Estimate the variance.", workingLatex: "\\sigma^2=\\dfrac{\\sum fx^2}{n}-\\bar{x}^2=\\dfrac{13787.5}{60}-12.41\\overline{6}^{\\,2}=229.79-154.17=75.62", explanation: "Use the form \\(\\sigma^2=\\frac{\\sum fx^2}{n}-\\bar{x}^2\\). Use the unrounded mean here; rounding \\(\\bar{x}\\) to 12.4 too soon shifts the variance noticeably." },
        { stepNumber: 8, description: "Take the square root for the standard deviation.", workingLatex: "\\sigma=\\sqrt{75.62}=8.70\\text{ mm (3 s.f.)}", explanation: "The standard deviation is the positive square root of the variance, in the same units (mm)." },
        { stepNumber: 9, description: "Interpret the skewness from mean vs median.", workingLatex: "\\bar{x}>\\text{median}", explanation: "When the mean exceeds the median the distribution has positive (right) skew: a longer tail of high rainfall days pulls the mean above the median. A common slip is to state the skew direction without giving the 'tail' reason." },
        { stepNumber: 10, description: "Explain why the answers are only estimates.", workingLatex: "", explanation: "The raw data are grouped, so the exact rainfall within each class is unknown; using mid-points assumes every reading sits at the centre of its class, which is generally untrue and introduces error into both the mean and the standard deviation." }
      ],
  ...
  <Text x={58} y={0.24} attach="s">58</Text>
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Read the quartiles and median from the diagram.", workingLatex: "Q_1=22,\\quad \\text{median}=27,\\quad Q_3=34", explanation: "The two box edges give \\(Q_1\\) and \\(Q_3\\); the line inside the box gives the median. Read carefully against the scale to avoid mis-reading the values." },
        { stepNumber: 2, description: "Compute the interquartile range.", workingLatex: "\\text{IQR}=Q_3-Q_1=34-22=12", explanation: "The IQR is the width of the box. A common slip is to use the whole range (max − min) instead." },
        { stepNumber: 3, description: "Compute \\(1.5\\times\\text{IQR}\\).", workingLatex: "1.5\\times\\text{IQR}=1.5\\times12=18", explanation: "This is the allowance added beyond the upper quartile before a value counts as an outlier." },
        { stepNumber: 4, description: "Find the upper outlier fence and conclude.", workingLatex: "Q_3+1.5\\times\\text{IQR}=34+18=52", explanation: "Any value above this fence is an outlier, so \\(t>52\\) is an outlier, as required to show." },
        { stepNumber: 5, description: "Test the largest value of 58 against the fence.", workingLatex: "58>52", explanation: "Since 58 exceeds the upper fence of 52, the time of 58 minutes is an outlier. Justify by an explicit comparison, not just by stating the conclusion." },
        { stepNumber: 6, description: "Compare the two halves of the box for skewness.", workingLatex: "Q_3-\\text{med}=34-27=7,\\quad \\text{med}-Q_1=27-22=5", explanation: "The upper box section (7) is wider than the lower (5), and the upper whisker is far longer than the lower, so the distribution is positively skewed." },
        { stepNumber: 7, description: "State the skewness conclusion.", workingLatex: "Q_3-\\text{med}>\\text{med}-Q_1", explanation: "Both the wider upper box and the longer upper whisker point to positive (right) skew. Quote the inequality as the supporting reason." },
        { stepNumber: 8, description: "Evaluate the commuter's claim.", workingLatex: "Q_1=22,\\ Q_3=34", explanation: "The middle 50% of commuters lie between \\(Q_1=22\\) and \\(Q_3=34\\) minutes by definition of the quartiles, so the diagram does support the claim that half take between 22 and 34 minutes." }
      ],
  ...
  <Polygon points={[[180,0],[180,0.25],[200,0.25],[200,0]]} color="var(--mafs-fg-blue)" fillOpacity={0.2} />
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find each class width.", workingLatex: "\\text{widths}=10,\\ 5,\\ 5,\\ 10,\\ 20", explanation: "Width is upper boundary minus lower boundary. The classes are unequal, so each width must be found separately before any density is computed." },
        { stepNumber: 2, description: "Compute frequency density = frequency ÷ class width.", workingLatex: "\\tfrac{8}{10}=0.8,\\ \\tfrac{15}{5}=3.0,\\ \\tfrac{22}{5}=4.4,\\ \\tfrac{10}{10}=1.0,\\ \\tfrac{5}{20}=0.25", explanation: "Dividing each frequency by its width gives the bar height. A common slip is to use a common width and so distort the unequal bars." },
        { stepNumber: 3, description: "List the mid-points for the mean.", workingLatex: "x=155,\\ 162.5,\\ 167.5,\\ 175,\\ 190", explanation: "Mid-point of each class, e.g. \\(\\tfrac{160+165}{2}=162.5\\). Note the unequal-width classes still take the centre of their own interval." },
        { stepNumber: 4, description: "Compute \\(\\sum fx\\).", workingLatex: "\\sum fx=8(155)+15(162.5)+22(167.5)+10(175)+5(190)=1240+2437.5+3685+1750+950=10062.5", explanation: "Multiply each frequency by its mid-point and add. Listing the five products lets you check the total term by term." },
        { stepNumber: 5, description: "Estimate the mean.", workingLatex: "\\bar{h}=\\dfrac{\\sum fx}{n}=\\dfrac{10062.5}{60}=167.7\\text{ cm (1 d.p.)}", explanation: "Divide by \\(n=60\\), the total frequency. The result lies inside the heaviest class, as expected." },
        { stepNumber: 6, description: "Form cumulative frequencies and locate the median class.", workingLatex: "\\text{cf}: 8,\\ 23,\\ 45,\\ 55,\\ 60;\\quad \\tfrac{n}{2}=30", explanation: "The 30th value lies in \\(165\\le h<170\\), since the cumulative frequency is 23 at 165 and reaches 45 at 170, so 30 falls inside this class." },
        { stepNumber: 7, description: "Apply linear interpolation for the median.", workingLatex: "\\text{median}=165+\\dfrac{30-23}{22}\\times5=165+\\dfrac{7}{22}\\times5=165+1.59=166.6\\text{ cm}", explanation: "Add to the lower boundary the fraction \\(\\frac{30-23}{22}\\) of the class width 5. A common slip is using the class frequency from the wrong row, or the wrong cumulative frequency before the class." },
        { stepNumber: 8, description: "Address the modal-class error.", workingLatex: "\\text{fd}_{[150,160)}=0.8<\\text{fd}_{[165,170)}=4.4", explanation: "On a histogram with unequal widths the mode is the class with the greatest frequency density, not the one that merely looks tall or has a large frequency. The greatest density is 4.4, so the modal class is \\(165\\le h<170\\), not \\(150\\le h<160\\)." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "List the mid-points.", workingLatex: "x=2,\\ 6,\\ 10,\\ 14,\\ 18", explanation: "Mid-point of each 4-minute class, e.g. \\(\\tfrac{0+4}{2}=2\\). Equal widths here, so the mid-points step up by 4 each time." },
        { stepNumber: 2, description: "Compute \\(\\sum fx\\).", workingLatex: "\\sum fx=6(2)+14(6)+10(10)+8(14)+2(18)=12+84+100+112+36=344", explanation: "Multiply each mid-point by its frequency and add. Listing terms makes the total easy to verify." },
        { stepNumber: 3, description: "Show the mean equals 8.6.", workingLatex: "\\bar{t}=\\dfrac{\\sum fx}{n}=\\dfrac{344}{40}=8.6", explanation: "Dividing 344 by the total frequency 40 gives exactly 8.6 minutes, as required. For a 'show that' you must reach the stated value precisely." },
        { stepNumber: 4, description: "Compute \\(\\sum fx^{2}\\).", workingLatex: "\\sum fx^{2}=6(2^2)+14(6^2)+10(10^2)+8(14^2)+2(18^2)=24+504+1000+1568+648=3744", explanation: "Square each mid-point first, then multiply by frequency, then sum. A common slip is squaring \\(fx\\) rather than \\(x\\)." },
        { stepNumber: 5, description: "Compute the variance.", workingLatex: "\\sigma^2=\\dfrac{\\sum fx^2}{n}-\\bar{t}^2=\\dfrac{3744}{40}-8.6^2=93.6-73.96=19.64", explanation: "Use \\(\\sigma^2=\\frac{\\sum fx^2}{n}-\\bar{t}^2\\). Subtract the square of the mean, not the mean of the squares minus the mean." },
        { stepNumber: 6, description: "Take the square root to show the standard deviation.", workingLatex: "\\sigma=\\sqrt{19.64}=4.43\\text{ min (3 s.f.)}", explanation: "The positive square root confirms the required \\(\\sigma\\approx4.43\\) minutes." },
        { stepNumber: 7, description: "Compute the 'unusually long' threshold.", workingLatex: "\\bar{t}+2\\sigma=8.6+2(4.43)=8.6+8.86=17.46", explanation: "Two standard deviations above the mean is about 17.46 minutes. Use the unrounded \\(\\sigma\\) if available to avoid drift, but here 4.43 is sufficient." },
        { stepNumber: 8, description: "State the smallest whole-number time.", workingLatex: "t>17.46\\Rightarrow t=18", explanation: "The smallest whole number of minutes that exceeds 17.46 is 18 minutes. A common slip is rounding 17.46 down to 17, which does not satisfy the strict 'more than' condition." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        ...
        { stepNumber: 2, description: "Compute \\(1.5\\times\\text{IQR}\\).", workingLatex: "1.5\\times17=25.5", explanation: "This allowance is added beyond each quartile to form the fences." },
        { stepNumber: 3, description: "Find the lower and upper outlier boundaries.", workingLatex: "Q_1-25.5=41-25.5=15.5,\\qquad Q_3+25.5=58+25.5=83.5", explanation: "Values below 15.5 or above 83.5 are outliers. Subtract at the lower quartile and add at the upper; mixing these up is a common slip." },
        { stepNumber: 4, description: "Test the minimum value.", workingLatex: "20>15.5", explanation: "The minimum 20 lies above the lower fence 15.5, so it is not an outlier." },
        { stepNumber: 5, description: "Test the maximum value.", workingLatex: "95>83.5", explanation: "The maximum 95 exceeds the upper fence 83.5, so it is an outlier." },
        { stepNumber: 6, description: "Compare quartile distances for skewness.", workingLatex: "Q_3-\\text{med}=58-49=9,\\quad \\text{med}-Q_1=49-41=8", explanation: "The upper quartile is slightly further from the median than the lower quartile, indicating mild positive skew." },
        { stepNumber: 7, description: "Compare the mean and median.", workingLatex: "\\bar{m}=52.3>\\text{median}=49", explanation: "The mean lies above the median, which independently indicates positive skew with a tail towards heavier parcels." },
        { stepNumber: 8, description: "Combine the evidence on skewness.", workingLatex: "Q_3-\\text{med}>\\text{med}-Q_1\\ \\text{and}\\ \\bar{m}>\\text{median}", explanation: "Both the quartile comparison and mean > median point to positive (right) skew, consistent with the high outlier at 95 kg stretching the upper tail." },
        { stepNumber: 9, description: "Comment on removing outliers.", workingLatex: "", explanation: "Removing the 95 kg parcel would lower the quoted average and may misrepresent the parcels customers actually send. Outliers should only be deleted if they are genuine errors; a 95 kg parcel may well be legitimate, so removing it could give a misleadingly low average. Quoting the median (49 kg), which is resistant to outliers, would be a more appropriate 'typical' figure." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Form the cumulative frequencies.", workingLatex: "\\text{cf}: 8,\\ 25,\\ 50,\\ 65,\\ 70", explanation: "Running totals up to the upper boundary of each class. The final cf must equal \\(n=70\\) as a check." },
        ...
        { stepNumber: 3, description: "Interpolate for the median and show it equals 44.", workingLatex: "\\text{median}=40+\\dfrac{35-25}{25}\\times10=40+\\dfrac{10}{25}\\times10=40+4=44", explanation: "Add to the lower boundary 40 the fraction \\(\\frac{35-25}{25}\\) of the width 10. This gives exactly 44 cm, as required to show." },
        { stepNumber: 4, description: "Locate and interpolate for \\(Q_1\\).", workingLatex: "\\tfrac{n}{4}=17.5\\in[30,40):\\ Q_1=30+\\dfrac{17.5-8}{17}\\times10=30+\\dfrac{9.5}{17}\\times10=35.6\\text{ cm}", explanation: "The 17.5th value falls in \\(30\\le L<40\\) (cf 8 at 30, 25 at 40). Use the cf before the class (8) and the class frequency (17)." },
        { stepNumber: 5, description: "Locate and interpolate for \\(Q_3\\).", workingLatex: "\\tfrac{3n}{4}=52.5\\in[50,60):\\ Q_3=50+\\dfrac{52.5-50}{15}\\times10=50+\\dfrac{2.5}{15}\\times10=51.7\\text{ cm}", explanation: "The 52.5th value falls in \\(50\\le L<60\\) (cf 50 at 50, 65 at 60). A common slip is using \\(\\frac{n}{4}\\) instead of \\(\\frac{3n}{4}\\) for the upper quartile." },
        { stepNumber: 6, description: "Compute the IQR.", workingLatex: "\\text{IQR}=Q_3-Q_1=51.7-35.6=16.1\\text{ cm}", explanation: "Subtract the lower quartile from the upper quartile." },
        { stepNumber: 7, description: "Compare IQR with the range.", workingLatex: "", explanation: "The range uses only the two extreme values and is heavily affected by any unusually short or long leaf (an outlier). The IQR uses the middle 50% of the data, so it is more resistant to extremes and gives a more reliable picture of the typical spread." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find the sampling fraction.", workingLatex: "\\dfrac{\\text{sample}}{\\text{population}}=\\dfrac{80}{1000}=0.08", explanation: "Each stratum is sampled in proportion to its size, i.e. 8% of every station's records." },
        { stepNumber: 2, description: "Apply the fraction to each station.", workingLatex: "A:0.08\\times450=36,\\quad B:0.08\\times300=24,\\quad C:0.08\\times250=20", explanation: "Multiply each stratum size by 0.08. Here each gives a whole number, so no rounding is needed." },
        { stepNumber: 3, description: "Check the strata totals match the sample size.", workingLatex: "36+24+20=80", explanation: "The three allocations must sum to 80; if rounding had been required you would adjust one stratum so the total is exact." },
        { stepNumber: 4, description: "Find the systematic sampling interval for Station A.", workingLatex: "k=\\dfrac{450}{36}=12.5\\approx12\\ (\\text{or }13)", explanation: "Divide the stratum size by the number required from it to get the interval \\(k\\)." },
        { stepNumber: 5, description: "Describe the systematic selection procedure.", workingLatex: "\\text{random start in }1..k,\\ \\text{then every }k\\text{th record}", explanation: "List the 450 Station A records in order, choose a random starting record between 1 and \\(k\\), then take every \\(k\\)th record thereafter until 36 are selected. State the interval (\\(\\approx12\\)) explicitly." },
        { stepNumber: 6, description: "State an advantage of stratification.", workingLatex: "", explanation: "A stratified sample guarantees each station is represented in proportion to its size. A simple random sample could, by chance, badly under- or over-represent a station, so stratification reduces sampling bias and usually gives a more accurate estimate of the overall mean." },
        { stepNumber: 7, description: "Explain why July-only is unrepresentative.", workingLatex: "", explanation: "Daily maximum temperature varies strongly with season. Using only July records reflects summer conditions, not the whole year, so the sample mean would over-estimate the true overall mean daily maximum temperature." },
        { stepNumber: 8, description: "Suggest an improvement.", workingLatex: "", explanation: "Spread the records across all months/seasons of the data set — for example stratify by month as well as by station, or sample throughout the year — so that seasonal variation is properly represented." }
      ],
  ...
  <Text x={55} y={0.24} attach="s">55</Text>
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "State the summary values for each group.", workingLatex: "A:\\ 8,15,20,27,40\\qquad B:\\ 10,22,30,38,55", explanation: "Group A (lower diagram): min 8, \\(Q_1\\) 15, median 20, \\(Q_3\\) 27, max 40. Group B (shown): min 10, \\(Q_1\\) 22, median 30, \\(Q_3\\) 38, max 55." },
        { stepNumber: 2, description: "Compare the medians in context.", workingLatex: "\\text{med}_B=30>\\text{med}_A=20", explanation: "Group B's median mark is 10 marks higher, so a typical Group B student scored higher than a typical Group A student." },
        { stepNumber: 3, description: "Compute and compare the interquartile ranges.", workingLatex: "\\text{IQR}_A=27-15=12,\\quad \\text{IQR}_B=38-22=16", explanation: "Group B has the larger IQR, so the middle 50% of Group B's marks are more spread out — Group B is less consistent than Group A." },
        { stepNumber: 4, description: "Comment on skewness of Group A.", workingLatex: "Q_3-\\text{med}=7>\\text{med}-Q_1=5", explanation: "The upper box section and upper whisker are longer, so Group A is positively skewed." },
        { stepNumber: 5, description: "Comment on skewness of Group B.", workingLatex: "Q_3-\\text{med}=8=\\text{med}-Q_1=8;\\ \\text{upper whisker }17>\\text{lower }12", explanation: "Group B's box is symmetric, but the longer upper whisker (55−38=17 vs 22−10=12) gives overall mild positive skew." },
        { stepNumber: 6, description: "Test whether B beats A at every quartile.", workingLatex: "\\text{med}_B=30>\\text{med}_A=20,\\ Q_{1B}=22>Q_{3A}=27?\\ \\text{No, but } Q_{1B}=22>Q_{1A}=15", explanation: "Compare like-for-like statistics: B's minimum, both quartiles, median and maximum all exceed A's corresponding values, so B sits higher throughout." },
        { stepNumber: 7, description: "Evaluate the teacher's claim.", workingLatex: "\\text{all of }\\{min,Q_1,med,Q_3,max\\}_B>\\{\\cdot\\}_A", explanation: "Since every summary value for B exceeds the matching value for A, the diagram supports the claim that Group B performed better overall, though Group A's marks were more consistent." },
        { stepNumber: 8, description: "State a limitation of box plots.", workingLatex: "", explanation: "A box plot does not show the number of students in each group or the shape of the distribution within each quartile (clustering or gaps), so 'overall' comparisons may hide important detail such as differing group sizes." }
      ],
  ...
  <Polygon points={[[25,0],[25,0.33],[40,0.33],[40,0]]} color="var(--mafs-fg-blue)" fillOpacity={0.2} />
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "Find each class width.", workingLatex: "\\text{widths}=5,\\ 5,\\ 5,\\ 10,\\ 15", explanation: "Upper minus lower boundary for each class. The last two classes are wider, which matters for both the densities and the part (b) interpolation." },
        { stepNumber: 2, description: "Compute frequency densities.", workingLatex: "\\tfrac{10}{5}=2.0,\\ \\tfrac{24}{5}=4.8,\\ \\tfrac{30}{5}=6.0,\\ \\tfrac{16}{10}=1.6,\\ \\tfrac{5}{15}=0.33", explanation: "Divide each frequency by its class width to get bar heights. A common slip is to forget the wider widths and over-state the last two bars." },
        ...
        { stepNumber: 4, description: "Add the patients in the final class.", workingLatex: "8+5=13", explanation: "Every patient in \\(25\\le w<40\\) waited more than 20 minutes, so add all 5. About 13 patients waited more than 20 minutes." },
        { stepNumber: 5, description: "List mid-points and compute \\(\\sum fx\\) for the mean.", workingLatex: "x=2.5,7.5,12.5,20,32.5;\\ \\sum fx=25+180+375+320+162.5=1062.5", explanation: "Mid-point of each class times its frequency, summed. The wider last classes take the centre of their own (wider) interval, e.g. \\(\\tfrac{25+40}{2}=32.5\\)." },
        { stepNumber: 6, description: "Estimate the mean and compare with the advert.", workingLatex: "\\bar{w}=\\dfrac{1062.5}{85}=12.5>10", explanation: "The estimated mean wait is about 12.5 minutes, noticeably more than the advertised 10 minutes, so 'a typical wait of about 10 minutes' understates the average and is not well justified." },
        { stepNumber: 7, description: "Explain why a single 'typical' value misleads.", workingLatex: "", explanation: "Waiting times spread from under 5 to nearly 40 minutes and the distribution is positively skewed, so no single value captures everyone's experience — many patients wait far longer than 10 minutes. Quoting only one figure hides this variability." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "List the mid-points.", workingLatex: "x=5,\\ 15,\\ 25,\\ 35,\\ 45", explanation: "Mid-point of each 10 mm class, e.g. \\(\\tfrac{0+10}{2}=5\\). Equal widths, so mid-points rise by 10 each time." },
        { stepNumber: 2, description: "Compute \\(\\sum fx\\).", workingLatex: "\\sum fx=3(5)+7(15)+15(25)+10(35)+5(45)=15+105+375+350+225=1070", explanation: "Multiply each mid-point by its frequency and add. Listing terms makes the total checkable." },
        { stepNumber: 3, description: "Show the mean equals 26.75.", workingLatex: "\\bar{r}=\\dfrac{\\sum fx}{n}=\\dfrac{1070}{40}=26.75", explanation: "Dividing 1070 by the total frequency 40 gives exactly 26.75 mm, as required for the 'show that'." },
        { stepNumber: 4, description: "Compute \\(\\sum fx^{2}\\).", workingLatex: "\\sum fx^{2}=3(25)+7(225)+15(625)+10(1225)+5(2025)=75+1575+9375+12250+10125=33400", explanation: "Square each mid-point first, then multiply by frequency and sum. Squaring \\(fx\\) instead of \\(x\\) is a common slip." },
        { stepNumber: 5, description: "Compute the variance.", workingLatex: "\\sigma^2=\\dfrac{\\sum fx^2}{n}-\\bar{r}^2=\\dfrac{33400}{40}-26.75^2=835-715.5625=119.4375", explanation: "Use \\(\\sigma^2=\\frac{\\sum fx^2}{n}-\\bar{r}^2\\) with the exact mean to avoid rounding error." },
        { stepNumber: 6, description: "Take the square root to show the standard deviation.", workingLatex: "\\sigma=\\sqrt{119.4375}=10.93=10.9\\text{ mm (3 s.f.)}", explanation: "The positive square root confirms \\(\\sigma\\approx10.9\\) mm, as required." },
        { stepNumber: 7, description: "Compute the upper outlier boundary.", workingLatex: "\\bar{r}+2\\sigma=26.75+2(10.93)=26.75+21.86=48.6", explanation: "Two standard deviations above the mean is about 48.6 mm. Use the unrounded \\(\\sigma=10.93\\) so the boundary is accurate to one decimal place." },
        { stepNumber: 8, description: "Decide whether 49 mm is an outlier.", workingLatex: "49>48.6", explanation: "Since 49 mm exceeds the upper boundary of 48.6 mm, the value would just be classed as an outlier. The margin is small, so working with an unrounded \\(\\sigma\\) is important here." },
        { stepNumber: 9, description: "Comment on suitability with a 120 mm month.", workingLatex: "", explanation: "A single value of 120 mm is far above the rest of the data and would substantially inflate both the mean and (especially) the standard deviation, since both use every value and are not resistant to extremes. The median and interquartile range would give a more reliable summary of typical monthly rainfall in that case." }
      ],
  ...
  },
  {
    ...
    workedSolution: {
      steps: [
        { stepNumber: 1, description: "List the mid-points.", workingLatex: "x=15,\\ 25,\\ 35,\\ 45,\\ 55", explanation: "Mid-point of each 10-minute class, shared by both routes." },
        { stepNumber: 2, description: "Estimate the Route X mean.", workingLatex: "\\bar{x}_X=\\dfrac{9(15)+18(25)+15(35)+6(45)+2(55)}{50}=\\dfrac{135+450+525+270+110}{50}=\\dfrac{1490}{50}=29.8", explanation: "Sum \\(fx=1490\\) for Route X and divide by 50. Listing the five products lets you verify the total." },
        { stepNumber: 3, description: "Estimate the Route Y mean.", workingLatex: "\\bar{x}_Y=\\dfrac{4(15)+14(25)+20(35)+9(45)+3(55)}{50}=\\dfrac{60+350+700+405+165}{50}=\\dfrac{1680}{50}=33.6", explanation: "Sum \\(fx=1680\\) for Route Y and divide by 50. Use Route Y's own frequencies, not X's." },
        { stepNumber: 4, description: "Compute \\(\\sum fx^2\\) for Route X.", workingLatex: "\\sum fx^2=9(225)+18(625)+15(1225)+6(2025)+2(3025)=2025+11250+18375+12150+6050=49850", explanation: "Square each mid-point, multiply by Route X's frequency, then sum. Squaring \\(x\\) (not \\(fx\\)) is the key point." },
        { stepNumber: 5, description: "Compute the Route X variance.", workingLatex: "\\sigma_X^2=\\dfrac{\\sum fx^2}{n}-\\bar{x}_X^2=\\dfrac{49850}{50}-29.8^2=997-888.04=108.96", explanation: "Use \\(\\sigma^2=\\frac{\\sum fx^2}{n}-\\bar{x}^2\\). Subtract the square of the mean." },
        { stepNumber: 6, description: "Take the square root for the Route X standard deviation.", workingLatex: "\\sigma_X=\\sqrt{108.96}=10.4\\text{ min (3 s.f.)}", explanation: "The positive square root gives the spread of Route X's times in minutes." },
        { stepNumber: 7, description: "Compare the routes in context.", workingLatex: "\\bar{x}_X=29.8<\\bar{x}_Y=33.6", explanation: "Route X has the smaller mean journey time — about 4 minutes quicker on average — so on average Route X is faster. The most common class is \\(20\\le t<30\\) for X but \\(30\\le t<40\\) for Y, consistent with Y being slower." },
        { stepNumber: 8, description: "Advise the reliability-focused commuter.", workingLatex: "", explanation: "Reliability is about consistency, measured by spread (standard deviation or IQR), not just the average. Route X is faster on average, but to compare reliability the commuter needs the spread of both routes. The further information required is the standard deviation (or IQR) of Route Y, so the spreads can be compared — a smaller spread means a more predictable journey time." }
      ],
