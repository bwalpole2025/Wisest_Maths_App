 ...
 *
 * The marking service depends only on `MarkGrader`, so the LLM vendor (Anthropic
 * today) can be swapped. The grader takes the question, the official mark scheme,
...
export type GraderErrorCode =
  ...
  | "anthropic_http" // other HTTP / network failure
