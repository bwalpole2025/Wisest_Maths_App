...
>
> **Recognition egress (SECRETS-1 / DEPENDENCIES-5):** `lib/services/recognitionService.ts` sends a stored submission image to **Mathpix** (`api.mathpix.com/v3/text`) for OCR. Credentials `MATHPIX_APP_ID`/`MATHPIX_APP_KEY` live in the env/config layer (`lib/env.ts`), are read only via `env.*` (never `process.env` in the service), are sent as request headers, and are **never logged** (failures log only `submissionId` + an error code; the stored `submissions.failure_reason` is a short non-secret string). DB writes for this session-less step use the **service-role** client (`lib/db/service.ts`), which the migration grants on `submissions`/`recognitions`.
>
> **Marking egress (SECRETS-1 / DEPENDENCIES-5):** `lib/services/markingService.ts` sends the question, the mark scheme, and the student's **confirmed** working to **Anthropic** (`api.anthropic.com/v1/messages`) for M/A/B marking. `ANTHROPIC_API_KEY` lives in `lib/env.ts`, is read only via `env.*`, is sent as the `x-api-key` header, and is **never logged** (failures store only `marking:<code>` in `submissions.failure_reason`). The marker reads **only** `transcription_confirmations.confirmed_latex` — never `recognitions.raw_latex`. The model id is `env.ANTHROPIC_MARKER_MODEL` (config, not hard-coded). Per-mark allocation is persisted as `advisory=true`; low-confidence marks are surfaced for human review, not counted as final.
