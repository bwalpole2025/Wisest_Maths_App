---
name: wisest-undergrad-question-generator
description: "Generate undergraduate engineering / STEM mathematics question banks for the Wisest AI Tutor platform as TypeScript files matching the project's Question[] schema. Trigger whenever the user asks for undergraduate (first/second-year) maths questions, an engineering-maths question bank, or content for a topic in the undergrad manifest — complex numbers, vectors, linear algebra, single-variable calculus, sequences & series, ODEs, multivariable / vector calculus, numerical methods — even when they don't explicitly mention Wisest, TypeScript, or the schema. Also trigger on undergrad topic codes like ucx3, ula9, uod9, umv7, or a request to produce a .ts file of `questions` for one of those. Produces 20 questions by default across Foundation / Standard / Challenge difficulty bands, each with a granular step-by-step worked solution in LaTeX."
---
...
as defined by `content/undergrad/manifest-v2.ts`. Each manifest entry gives
...
1. **Resolve the topic from the manifest.** The user passes a topic `code`
   (e.g. `cx3`). Look it up in `content/undergrad/manifest-v2.ts` for its
   ...
   already supplies.
2. **If `visual: true`, defer to `wisest-question-generator-with-diagrams`** instead
...
3. **Plan 3–6 skill clusters** that partition the topic's `scope` (same rationale as
...
4. **Generate 20 questions** across clusters and difficulty bands (mix below).
5. **Write the output** as a valid TypeScript file `<code>.ts` following
   `references/schema.md`, importing `Question` from `@/lib/types`.
6. **Save** to the undergraduate content directory of the repo and present it with
   `present_files`.
